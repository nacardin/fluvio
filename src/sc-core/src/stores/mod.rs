...
mod basic {
    ...
    where
        <Self as Spec>::Owner: K8ExtendedSpec,
    {
        // convert kubernetes objects into KV value
        fn convert_from_k8(
            k8_obj: K8Obj<Self::K8Spec>,
        ) -> Result<MetadataStoreObject<Self>, IoError>
        where
            ...
            Self::K8Spec: Into<Self>,
        {
            ...
            match result {
                Ok(key) => {
                    ...
                    let ctx = MetadataContext::default().with_ctx(k8_obj.metadata);
                    let loca_kv =
                        MetadataStoreObject::new(key, local_spec, local_status).with_kv_ctx(ctx);
